...
def run_tests(train, test, trainExtra, testExtra, scaler, sarima_order_parameters, sarima_season_parameters, num_predicitons, location, data_split):   
    # Output filename
    ...
    for s_seq in prediction_list:
        ...
        for sarima_order_parameter in sarima_order_parameters:
            for sarima_season_parameter in sarima_season_parameters:
                ...
                try:
                    broke=0
                    model = SARIMAX(endog=history, order=(sarima_order_parameter[0],sarima_order_parameter[1],sarima_order_parameter[2]),seasonal_order =(sarima_season_parameter[0],sarima_season_parameter[1],sarima_season_parameter[2],sarima_season_parameter[3]),enforce_stationarity=False, enforce_invertibility=False, exog=historyExtra)
                    model_fit = model.fit(disp=0)
                    output = model_fit.forecast(steps=num_predictions, exog=real_resultsExtra)
                ...
                if(broke==0):
                    ...
                    ## Save Results
                    figure_name = 'results_sarimax/'+outputfilename+'_sarimax(' + str(sarima_order_parameter[0])+ ',' + str(sarima_order_parameter[1]) + ',' + str(sarima_order_parameter[2]) + ')('+str(sarima_season_parameter[0]) + ','+str(sarima_season_parameter[1]) + ','+str(sarima_season_parameter[2]) + ','+str(sarima_season_parameter[3]) + ','+ ')_predictions_' + str(num_predictions)+'_crossvalidation_'+ str(data_split)+'_test_' + str(s_seq) +'.png' 
                    ...
                    results_dataset_raw.to_csv('results_sarimax/'+outputfilename+'_raw_'+'_sarimax(' + str(sarima_order_parameter[0])+ ',' + str(sarima_order_parameter[1]) + ',' + str(sarima_order_parameter[2]) + ')('+str(sarima_season_parameter[0]) + ','+str(sarima_season_parameter[1]) + ','+str(sarima_season_parameter[2]) + ','+str(sarima_season_parameter[3]) + ','+')_predictions_' + str(num_predictions)+'_crossvalidation_'+ str(data_split)+'_test_' + str(s_seq) +'.csv')
...
for location in locations:
    ...
    for train_index, test_index in tscv.split(dt1):
        ...
        for num_predictions in predictions:
            ...
            results = run_tests(train1.copy(),test1.copy(), train1Extra.copy(),test1Extra.copy(), scaler, sarima_order_parameters, sarima_season_parameters, num_predictions, location, data_split)

            results_dataset=pd.concat([results_dataset,results], ignore_index=True)

        data_split += 1
                
results_dataset.to_csv('results_sarimax/SARIMAX_results_summary.csv', index=False, )
